...
test('seeded malformed-input differential checks against native Brotli', () => {
  ...
  if (!Number.isSafeInteger(count) || count < 1) {
    throw new RangeError('Invalid BROTLI_FUZZ_CASES.')
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (i % 5) {
      case 0: {
        input = randomBytes(random(80), random(0x7F_FF_FF_FF) + 1)
        break
      }
      case 1: {
      ...
      case 2: {
        input = concat(input, randomBytes(random(5000) + 1, random(0x7F_FF_FF_FF)))
        break
      }
      default: {
        for (let j = 0, n = random(3) + 1; j < n; j++) {
    ...
    try {
      native = nativeDecode(input, 65_536)
    } catch { /* Native rejection is the expected outcome for most mutations. */ }
    let actual: Uint8Array | undefined
    try {
      actual = decodeBrotli(input, {maxOutputLength: 65_536})
    } catch (error) {
    ...
    if (Boolean(actual) !== Boolean(native) || actual && native && !Buffer.from(actual).equals(native)) {
      throw new Error(`Differential mismatch at case ${i}: native=${native?.length}, JavaScript=${actual?.length}, input=${input.toHex()}`)
    }
  }
}, 120_000)
