import type {BrotliErrorCode} from '../src/main.ts'
...
export function randomBytes(length: number, seed = 0x24_3F_6A_88): Uint8Array<ArrayBuffer> {
...
export function nativeDecode(input: Uint8Array, maxOutputLength = 1_000_000): Uint8Array<ArrayBuffer> {
  ...
  const result = brotliDecompressSync(input, {
    info: true,
    maxOutputLength,
  }) as unknown as {
    buffer: Buffer
...
export function uncompressed(input: Uint8Array): Uint8Array<ArrayBuffer> {
  if (input.length < 1 || input.length > 65_536) {
    throw new RangeError('Invalid test block size.')
  }
...
export function metadata(input: Uint8Array): Uint8Array<ArrayBuffer> {
  if (input.length < 1 || input.length > 256) {
    throw new RangeError('Invalid test metadata size.')
  }
...
export function uncompressedBlocks(...blocks: Array<Uint8Array>): Uint8Array<ArrayBuffer> {
...
export function expectError(input: Uint8Array, code?: BrotliErrorCode, maxOutputLength?: number) {
  ...
  if (code) {
    expect((caught as BrotliDecodeError).code).toBe(code)
  }
}
