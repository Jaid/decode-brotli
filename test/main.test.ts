...
import decodeBrotli, {BrotliDecodeError, decodeBrotli as namedDecode} from '../src/main.ts'
...
describe('public API', () => {
  ...
  test('empty stream is valid, empty input is not', () => {
    for (const value of [0x06, 0x3B]) {
      expect(decodeBrotli(Uint8Array.of(value))).toEqual(new Uint8Array)
    }
  ...
  test('accepts ArrayBuffer, Buffer, typed arrays and DataView with exact view boundaries', () => {
    ...
    const inputs: Array<BrotliInput> = [
      [...hello].buffer,
      Buffer.from(hello),
    ...
    for (const input of inputs) {
      expect(utf8.decode(decodeBrotli(input))).toBe('hello world')
    }
    const even = uncompressed(Uint8Array.of(42, 42))
    expect(utf8.decode(decodeBrotli(new Uint16Array(even.buffer)))).toBe('**')
  })
...
describe('resource limits', () => {
  ...
  test.each([65_535, 65_536, 65_537, 131_072])('exact output buffer boundary: %d', length => {
  ...
  test('rejects truncation at every byte of varied streams', () => {
    for (const compressed of [hello, compress(sampleText.repeat(8), 11), uncompressed(randomBytes(200)), metadata(randomBytes(30))]) {
      for (let length = 0; length < compressed.length; length++) {
        expectError(compressed.subarray(0, length))
      }
...
describe('bit-reader and ring-buffer boundaries', () => {
  test.each([1, 2, 3, 4090, 4091, 4092, 4093, 4094, 4095, 4096, 4097, 8192, 16_383, 16_384, 16_385, 65_535, 65_536])('raw block length %d', length => {
...
describe('native encoder interoperability', () => {
  test.each(Array.from({length: 12}, (_, i) => i))('all qualities: %d', quality => {
    for (const input of [new Uint8Array, randomBytes(12_000), text.encode(sampleText.repeat(300)), new Uint8Array(70_000).fill(255)]) {
  ...
  test('deterministic randomized data, entropy and sizes', () => {
    for (let seed = 1; seed <= 100; seed++) {
      const input = randomBytes(seed * 1543 % 100_003, seed)
